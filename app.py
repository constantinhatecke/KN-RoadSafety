"""
K+N Road Logistics — Live Operations Dashboard v4
--------------------------------------------------
Interactive dashboard with:
  - Sidebar risk filter (All / High / Medium / Clear)
  - Waypoints visible on map as colored dots
  - High-risk areas shown as diamonds
  - Severity-gated high-risk area logic (no more false positives)
  - Click corridor name to jump to detail
"""
...
<style>
  ...
  [data-testid="stSidebar"] .stRadio > label {{ color: {KN_LIGHT} !important; font-size: 13px !important; }}
  [data-testid="stSidebar"] [data-testid="stMarkdownContainer"] p {{ color: {KN_LIGHT} !important; font-size: 12px !important; }}
  ...
  /* Header */
  .kn-header {{ padding-bottom: 14px; border-bottom: 2px solid {KN_BLUE}; margin-bottom: 18px; display: flex; align-items: flex-end; justify-content: space-between; }}
  ...
  /* Alert */
  .kn-alert {{ background: rgba(232,57,74,0.12); border: 1px solid rgba(232,57,74,0.35); border-left: 4px solid {RISK_HIGH}; color: #FFB8BE !important; padding: 12px 16px; font-size: 14px; font-weight: 600; margin-bottom: 18px; }}
  ...
  /* Worst point callout */
  .worst-point-box {{ background: rgba(232,57,74,0.08); border: 1px solid rgba(232,57,74,0.25); border-left: 3px solid {RISK_HIGH}; padding: 9px 12px; margin: 8px 0; font-size: 13px; }}
  ...
  .wp-name {{ color: {KN_WHITE}; font-weight: 600; min-width: 150px; }}
  .wp-tag-hr {{ background: rgba(232,57,74,0.15); color: {RISK_HIGH}; border: 1px solid rgba(232,57,74,0.3); padding: 1px 7px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; white-space:nowrap; }}
  .wp-tag-iv {{ background: rgba(0,153,204,0.12); color: {KN_BLUE}; border: 1px solid rgba(0,153,204,0.3); padding: 1px 7px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; white-space:nowrap; }}
  .wp-tag-ep {{ background: rgba(200,216,232,0.08); color: {KN_LIGHT}; border: 1px solid {KN_BORDER}; padding: 1px 7px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; white-space:nowrap; }}
  .wp-risk-high   {{ color: {RISK_HIGH}; font-weight: 700; min-width:60px; }}
  .wp-risk-medium {{ color: {RISK_MED};  font-weight: 700; min-width:60px; }}
  .wp-risk-low    {{ color: {RISK_LOW};  font-weight: 600; min-width:60px; }}
  ...
  .stRadio [data-testid="stMarkdownContainer"] {{ margin-bottom: 0; }}
</style>
...
def run_pipeline():
    ...
    corridors     = [
        assess_corridor_risk(c, weather_map, corridor_wps.get(c["id"], []), country_data)
...
# ════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown(f"<div style='font-size:18px;font-weight:700;color:{KN_WHITE};padding-bottom:4px;'>K+N Operations</div>", unsafe_allow_html=True)
    st.markdown(f"<div style='font-size:10px;color:{KN_DIM};text-transform:uppercase;letter-spacing:1.5px;margin-bottom:20px;'>Road Logistics Dashboard</div>", unsafe_allow_html=True)

    st.markdown(f"<div style='font-size:10px;font-weight:700;color:{KN_DIM};text-transform:uppercase;letter-spacing:1.5px;margin-bottom:8px;'>Filter by Risk</div>", unsafe_allow_html=True)
    risk_filter = st.radio(
        label="risk_filter", label_visibility="collapsed",
        options=["🔴🟡🟢  All Corridors", "🔴  High Risk Only", "🟡  Medium Risk Only", "🟢  Clear Only"],
        index=0,
    )

    st.markdown("<hr style='border-color:#1E4070;margin:20px 0;'>", unsafe_allow_html=True)
    ...
    show_interval_wps = st.toggle("Show interval waypoints", value=True)
    show_hr_areas     = st.toggle("Show high-risk areas", value=True)
    show_hub_labels   = st.toggle("Show hub labels", value=True)
    ...
    if st.session_state.data:
        ...
        if avg_t is not None:
            st.markdown(
                f'<div style="margin-top:10px;padding:10px;background:{KN_CARD};border-left:3px solid #66CCFF;">'
...
filtered_corridors = [c for c in corridors if active_filter is None or c["risk_level"] == active_filter]
...
if st.session_state.last_fetched:
    ...
    st.markdown(
        f'<div class="kn-ts">'
        f'<span style="color:{RISK_LOW};font-size:9px">●</span>&nbsp; '
        f'Live data &nbsp;·&nbsp; Refreshed: <strong style="color:{KN_WHITE}">{st.session_state.last_fetched}</strong>'
...
if high_count > 0:
    ...
    d_str   = f"&nbsp;·&nbsp; Primary: <strong>{disp.replace('_',' ').title()}</strong>" if disp else ""
    hr_str  = f"&nbsp;·&nbsp; <strong>{hr_triggers} high-risk area{'s' if hr_triggers!=1 else ''}</strong> triggered" if hr_triggers else ""
    st.markdown(f'<div class="kn-alert">⚠&nbsp;&nbsp;{high_count} corridor{"s" if high_count>1 else ""} at HIGH RISK{d_str}{hr_str}</div>', unsafe_allow_html=True)

# KPIs — always show full network stats, not filtered
...
kpi(k5, "Avg Network Temp",    f"{avg_t}°C" if avg_t else "N/A",   "Across all hubs",       "kpi-temp")

# Filter badge
if active_filter:
    badge_color = COLOR_MAP[active_filter]
    st.markdown(
        f'<div style="margin:12px 0 4px;font-size:12px;color:{KN_DIM};">'
        f'Showing: <span style="color:{badge_color};font-weight:700;">'
        f'{active_filter.upper()} RISK</span> corridors only '
        f'({len(filtered_corridors)} of {len(corridors)})'
        f'</div>', unsafe_allow_html=True
    )
...
with left:
    ...
    # ── Corridor lines (filtered) ─────────────────────────────────
    for c in filtered_corridors:
        ...
        for c in filtered_corridors:
            for pt in c.get("all_points", []):
                if pt["type"] == "interval":
                    ...
                    iv_texts.append(
                        f"<b>{c['corridor_id']} — {pt['name']}</b><br>"
                        f"Risk: {pt['risk_level'].upper()}<br>"
                        f"Temp: {temp} &nbsp; Wind: {wind}<br>"
                        f"{cond}"
                    )
        if iv_lats:
            fig.add_trace(go.Scattergeo(
                lon=iv_lngs, lat=iv_lats,
                mode="markers",
                marker=dict(size=7, color=iv_colors, symbol="circle",
                            line=dict(width=1, color=KN_NAVY)),
                text=iv_texts,
    ...
    if show_hr_areas:
        ...
        if hr_lats:
            fig.add_trace(go.Scattergeo(
                ...
                mode="markers",
                marker=dict(size=11, color=hr_colors, symbol="diamond",
    ...
    fig.add_trace(go.Scattergeo(
        ...
        marker=dict(size=9, color=KN_BLUE, symbol="circle",
                    line=dict(width=1.5, color=KN_WHITE)),
    ...
    st.markdown(
        ...
        f'<b style="color:{KN_WHITE}">●</b> Hub &nbsp;·&nbsp; '
        f'<b style="color:{KN_WHITE}">◆</b> High-risk area &nbsp;·&nbsp; '
        f'<b style="color:{KN_WHITE}">●</b> Interval waypoint &nbsp;·&nbsp; '
        f'<span style="color:{RISK_HIGH}">■</span> High &nbsp;'
        f'<span style="color:{RISK_MED}">■</span> Medium &nbsp;'
        f'<span style="color:{RISK_LOW}">■</span> Clear'
        f'<br><em style="font-size:10px;">Hover over any point on the map for weather details</em></div>',
        unsafe_allow_html=True
    )
    ...
    fig2.update_layout(
        ...
        showlegend=False, height=200, margin=dict(l=0, r=0, t=8, b=0),
...
# ── Corridor cards ────────────────────────────────────────────────────────────
with right:
    filter_label = f" ({active_filter.upper()} only)" if active_filter else ""
    st.markdown(f'<div class="kn-section">Active Corridors{filter_label}</div>', unsafe_allow_html=True)

    badge = {"high": "🔴", "medium": "🟡", "low": "🟢"}
    bc    = {"high": RISK_HIGH, "medium": RISK_MED, "low": RISK_LOW}

    if not filtered_corridors:
        st.markdown(f'<div style="color:{KN_DIM};font-size:13px;padding:20px 0;">No corridors match the selected filter.</div>', unsafe_allow_html=True)
    ...
    for c in filtered_corridors:
        ...
        with st.expander(
            ...
            st.markdown(f"**Points assessed:** `{wp_count}` (endpoints + interval waypoints + high-risk areas)")

            # Worst point callout
            if worst and worst["risk_level"] != "low":
                type_label = (
                    "⚠ High-Risk Area"   if worst.get("type") == "high_risk"
            ...
            if c["disruptions"]:
                st.markdown("**Disruptions:** " + "  ".join([f"`{d.replace('_',' ')}`" for d in c["disruptions"]]))
            ...
            if all_pts:
                st.markdown(
                    ...
                    f'border-bottom:1px solid {KN_BORDER};">'
                    f'Point-by-point breakdown ({len(all_pts)} points)</div>',
                    unsafe_allow_html=True
                )
                for pt in all_pts:
                    pt_risk   = pt["risk_level"]
                    tag_class = ("wp-tag-hr" if pt["type"]=="high_risk"
                                 else "wp-tag-iv" if pt["type"]=="interval"
                                 else "wp-tag-ep")
                    tag_label = ("High-Risk Area" if pt["type"]=="high_risk"
                                 else "Interval" if pt["type"]=="interval"
                                 else "Endpoint")
                    w        = pt.get("weather", {})
...
hub_temps, seen = [], set()
for c in corridors:   # always show all hubs regardless of filter
...
<div class="kn-footer">
  ...
  <span>Kuehne+Nagel &nbsp;·&nbsp; Road Logistics &nbsp;·&nbsp; Operational Excellence &nbsp;·&nbsp; v4</span>
</div>
""", unsafe_allow_html=True)
